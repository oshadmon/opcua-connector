    ...
    def __init__(self, endpoint="127.0.0.1:4840", is_advanced:bool=False):
        ...
        if is_advanced is True:
            for tag in TAG_HIERARCHY:
                for index in TAG_HIERARCHY[tag]:
                    if index in IDX_VARIABLES:
                        TAG_HIERARCHY[tag][index] = IDX_VARIABLES[index]['alter_datatype']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--string-mode', choices=['int', 'short', 'long'], default=ua.VariantType.Int32, help='String NodeId mode')
    ...
    parser.add_argument('--data-tags', type=int, default=103, help='number of data points')
    args = parser.parse_args()
    if args.data_tags > 103:
        options = ['int', 'bool', 'float', 'double', 'char']
        new_tags = args.data_tags - 103
        for i in range(1, new_tags):
            tag_name = f'NT{i+new_tags}'
            IDX_VARIABLES[tag_name] = {'alter_datatype': random.choice(options), 'idx': i+103, 'min': 0.0, 'max': random.uniform(1, 100)}
            key_name = None
            key_size = 0
            for key in TAG_HIERARCHY:
                try:
                    if key_size == 0 or len(TAG_HIERARCHY[key]) < key_size:
                        key_name = key
                        key_size = len(TAG_HIERARCHY[key])
                except Exception:
                    pass
            TAG_HIERARCHY[key_name] = random.choice(['float', 'double'])
