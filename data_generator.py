...
def update_timestamp(file_content: list, iteration: int):
    min_ts = None
    max_ts = None
    ...
    increments = (time_diff.total_seconds() / max(1, len(file_content) - 1)) + (iteration * 60)

    # Assign new timestamps in order
    for i, row in enumerate(file_content):
        row['timestamp'] = (max_ts + datetime.timedelta(seconds=increments)).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
...
def worker(metadata: str, iteration: int):
    ...
    if iteration > 0:
        file_content = update_timestamp(file_content, iteration=iteration)
